/* Public-domain SHA-256 (FIPS 180-4). Compact, dependency-free, used
   on every platform so the Stasha crypto module needs no CommonCrypto
   or OpenSSL to hash. */
...
static void sts_sha256_block(sts_sha256_ctx *ctx, const uint8_t *p) {
...
void sts_sha256_final(uint8_t *digest, sts_sha256_ctx *ctx) {
    ...
    for (int j = 7; j >= 0; j--)
        ctx->buf[i++] = (uint8_t)(ctx->bitlen >> (j * 8));
    sts_sha256_block(ctx, ctx->buf);
...
void sts_sha256(const uint8_t *data, uint64_t len, uint8_t *digest) {
    sts_sha256_ctx ctx;
    sts_sha256_init(&ctx);
    sts_sha256_update(&ctx, data, len);
    sts_sha256_final(digest, &ctx);
}
