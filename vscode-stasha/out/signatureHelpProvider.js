...
exports.StashaSignatureHelpProvider = void 0;
...
function getSymsCached(doc) {
    const key = doc.uri.toString();
    const hit = cache.get(key);
    if (hit && hit.version === doc.version)
        return hit.symbols;
    const syms = (0, symbolProvider_1.getSymbols)(doc.uri.fsPath);
...
class StashaSignatureHelpProvider {
    provideSignatureHelp(doc, pos) {
        ...
        const matching = syms.filter(s => s.name === calleeName && (s.kind === 'function' || s.kind === 'method'));
...
exports.StashaSignatureHelpProvider = StashaSignatureHelpProvider;
